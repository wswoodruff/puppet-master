...
module.exports = class CopilotService extends Schmervice.Service {
    ...
    async startSession(page, config) {
        ...
        const clickInspectEvt = `${page.id}:clickInspect`;
        ...
        this.server.event({ name: clickInspectEvt, shared: true });
        ...
        this.server.events.on(updateStepEvt, ({ index: updateIndex, value }) => {
            ...
            // Remove the func signature from here yo
            ...
            const functionBody = value.split('{')[1].split('}')[0].replace(/[\n\u200b]/g, '').trim();

            console.log('functionBody', functionBody);

            status.draftBotScript[updateIndex] = new AsyncFunction('page', 'context', functionBody);

            console.log('status.draftBotScript[updateIndex]', status.draftBotScript[updateIndex]);
            broadcastStatus();
        });
        ...
        this.server.events.on(addStepEvt, ({ index: addIndex, value }) => {
            ...
            status.draftBotScript.splice(addIndex, 0, value);
            broadcastStatus();
        });
