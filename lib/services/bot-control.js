...
module.exports = class BotControlService extends Schmervice.Service {
    ...
    async copilot(page, config) {
        ...
        const openDevtoolsEvt = `${page.id}:openDevtools`;
        ...
        this.server.event({ name: openDevtoolsEvt, shared: true });
        ...
        // Inspect
        // Inspired by https://gideonpyzer.dev/blog/2018/12/30/using-puppeteer-to-interact-with-the-devtools-ui-hack
        this.server.events.on(openDevtoolsEvt, async (openDevtools) => {

            const { clickService } = this.server.services();

            const browser = clickService.getBrowser();
            ...
            const pageTargeUrl = `${wsEndpoint.replace('ws://', '').match(/.*(?=\/browser)/)[0]}/page/${pageTargetId}`;

            const pageDebuggingUrl = `chrome-devtools://devtools/bundled/devtools_app.html?ws=${pageTargeUrl}`;

            if (page.devtoolsPage) {
                await page.devtoolsPage.close();
            }

            // Open that devtools page maaaaan
            page.devtoolsPage = await clickService.getPage();
            page.devtoolsPage.goto(pageDebuggingUrl);
            // The installed extension may open a new tab so make sure we select the debugger UI tab
            // await this.devtoolsPage.bringToFront();
        });
